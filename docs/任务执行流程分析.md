...
`ForestEnergyCollector`（位于 `task/antForest/` 根目录）现在是门面，具体职责委托给 `task/antForest/collector/` 下的组件：
...
| RPC 并发限制 | `GlobalRpcRateLimiter`（rpc/intervallimit/GlobalRpcRateLimiter.kt:25）                           | 全局 `Semaphore` 限制同时在途 RPC 请求数，默认并发 2 |
...
| `GlobalRpcRateLimiter.maxConcurrency`（代码内硬编码，非配置项） | 2        | RPC 层同时在途请求数上限 |
| `forest.collectInterval` | 1000-1500ms（边界 50–10000ms，见 AntForest.kt:584/677）            | 收取间隔          |
